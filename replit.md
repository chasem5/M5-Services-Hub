...
- **CEO Command Center (`/ceo`)** — `super_admin`-only dashboard with live KPIs (revenue, pipeline, SA contract revenue, QCR, DSO, backlog, utilization); period toggle (daily/weekly/monthly) for all charts with dynamic SQL bucketing via `DATE_TRUNC`; Crew Capacity panel with actual vs scheduled hours, OT rate badge from timesheet data; labor cost 6-month spark chart; top 5 customers by revenue; AI summary generation; Data Upload card auto-detects: (1) BuildOps Service Agreement CSVs by "Agreement Number" + "Annual Contract Value" header → `POST /api/buildops/import-agreements` → upserts on `agreement_number`, (2) Timesheet CSVs by "visit id"/"labor rate group" header → `POST /api/buildops/import-timesheets` → upserts on `(visit_id, employee_name, labor_rate_group)`, skips zero-duration rows; other files added to AI context

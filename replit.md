...
- **CEO Command Center (`/ceo`)** — `super_admin`-only dashboard with live KPIs (revenue, pipeline, SA contract revenue, QCR, DSO, backlog, utilization); period toggle (daily/weekly/monthly) for all charts with dynamic SQL bucketing via `DATE_TRUNC`; Crew Capacity panel with actual vs scheduled hours, OT rate badge from timesheet data; labor cost 6-month spark chart; top 5 customers by revenue; AI summary generation; file upload card (BuildOps timesheet CSVs auto-detected by header and persisted to `buildops_timesheets` table; other files added to AI session context); `POST /api/buildops/import-timesheets` — parses CSV by column name, upserts on `(visit_id, employee_name, labor_rate_group)`, skips zero-duration rows, returns `{ inserted, updated, skipped }` summary

...
- **Lead/Pipeline Management** — **Two-track pipeline**: Relationship track (Met/Introduced → Reached Out → In Conversation → Ready for Proposal) and Deal track (Proposal Sent → Won → Lost) with visual separator in Kanban; `pipeline_stages.track` column (`relationship` | `deal`); Kanban cards auto-sorted by estimated value (high → low); Deal-track lead cards show BuildOps quote status badge inline; **BuildOps Quotes view** — preset filter showing all leads with `buildopsQuoteId` in 4 columns (Quote Needed / Quote Sent / Approved / Rejected); "Sync BuildOps Quotes" button to pull latest from BuildOps API; **Activity-first deal drawer** — 4-row compact read-only header (inline stage dropdown pill, ID, deal title, client·value·rep·expiry meta row, AI health chip + one-liner), persistent `QuickLogBar` (Note/Call/Email/Meeting/Site Visit pills with expandable textarea), tabs ordered Activity → Tasks → Files → Details; Activity tab shows `LeadActivityTab(hideComposer)` + linked emails; Tasks tab has AI suggested task banner first, then inline quick-add input (Enter to create), full-form toggle, task list; Files tab uses `AttachmentsPanel`; Details tab shows full AI analysis card, expiry nudge (if applicable), read-only field grid OR inline edit form (toggled by "Edit Deal" header button which also switches to Details tab); edit form has Save/Cancel buttons at top
...
- **Weekly Report AM Coaching Tool (`/weekly-report`)** — Weekly coaching report for Account Managers; auto-loads current week (Mon–Sun) with back/forward week navigation; 9-card Activity Snapshot (Emails, Calls, Other, Quotes Created, Quotes Sent, Tasks Done, Deals Won, Revenue Closed, Accounts Touched) + Active Proposals + Deals Lost; Revenue Snapshot section with MTD/QTD progress bars vs monthly/quarterly goals + MRR from active BuildOps service agreements; AI Coaching section powered by GPT-4o-mini generating 4 typed coaching items (win/focus/risk/tip) from live data; Customer Health table (all accounts where this AM is assigned — health status, MRR, open quotes, last contact); Proposal Aging table showing open pipeline deals with age-bucket badges (0-14d/15-30d/31-60d/60d+); Weekly Narrative (4 auto-save text areas: BD, Quotes, Jobs, SA); Spotlight pins (win or need-help items with optional value/note); **Action Items section** — AM can add/toggle/delete items + AI Generate (GPT-4o-mini, includes coaching context) on own report; completion progress badge; manager view shows items read-only (no add/toggle/delete); "Mark Ready for Review" button triggers in-app + push notifications to managers/admins; Chat panel (side drawer, polls every 20s, hidden in manager view) for AM ↔ Manager feedback; status badge (Draft/Ready); **Manager Full View** mode: `/weekly-report?userId=xxx&weekStart=yyy` shows "[First Name]'s Weekly Report" header, read-only banner with "Back to Team View" button, narrative as static text, action items as read-only checklist, no chat/edit controls; all 3 data endpoints support optional `?userId=xxx` param (manager/admin only); IDOR-safe action PATCH scoped to both actionId + reportId; DB tables: `weekly_reports`, `weekly_report_spotlights`, `weekly_report_messages`, `weekly_report_actions`; 14+ API endpoints under `/api/weekly-report`
- **Weekly Report Team View (`/weekly-report/team`)** — Manager/admin-only view; week navigation; summary stats (Total/Ready/Draft counts); filter tabs; expandable report cards per AM with narrative sections; inline chat panel per report for leaving manager feedback; **"View Full Report →" button** in expanded section of each card, navigates to manager full view (`/weekly-report?userId=xxx&weekStart=yyy`)
- **Estimating & Quoting Workspace (`/estimating`)** — AI-assisted estimating and quoting module for facility services. Opportunities list with KPI cards (total/draft/in-review/approved), search + status filter, create dialog with mode (Estimate/Quote), service lines multi-select, and scope of work. Workspace detail (`/estimating/:id`) has 3-column layout: line items editor with inline expand (labor hrs, material $, buffer %, waste %, difficulty %, margin %), below-the-line fees (profit/insurance, pct or fixed, with toggle switch), live math (buffered hrs × labor rate + material × waste × difficulty → direct cost → price at margin), grand total; AI analysis panel (scope risks, margin insights, missing items, competitiveness score) powered by GPT-4o; Submit for Review → creates sync run, sets status to `in_review`; Approve/Reject dialog for managers. DB tables: `opportunities`, `opportunity_sections`, `workspace_lines`, `btl_fees`, `ai_recommendations`, `sync_runs`, `buffer_rules`, `approval_rules`, `workspace_catalog_items` (16 seeded items: HVAC, Electrical, Plumbing, General). API: 20+ routes under `/api/opportunities` and `/api/estimating`. Sidebar "Estimating" section.

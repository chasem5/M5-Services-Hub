...
- **Email Sync** — Gmail OAuth integration with two-panel desktop layout; AI-powered analysis (summary, sentiment, tasks, stage suggestions) with user-identity-aware prompts; thread grouping by `gmailThreadId` with collapsible per-message expansion; CC detection (direction = "cc" stored in DB, muted treatment, no "Needs Response"); domain-based auto-linking to clients; AI connection suggestions; block individual sender or entire domain (@domain.com) via 3-option dismiss menu; blocked senders panel with unblock controls; email search (client-side, searches subject/sender/body/AI summary); clean email body processing (strips URLs, signatures, quoted replies, tracking artifacts); paragraph-based body rendering; auto-sync every 15 minutes; emails persist permanently in DB (30-day window only applies to Gmail API fetch)

...
const CARDS: Record<string, Card[]> = {
  ...
  proposal_sent: [
    ...
    { id: 11, title: "Facility Management – Harbor South", client: "Harbor Group", value: "$210k", rep: "RK", tag: "Facility", tagColor: "bg-teal-100 text-teal-700", date: "Nov 3" },
  ...
  return (
    <div className="bg-white rounded-lg border border-slate-200 shadow-sm hover:shadow-md hover:border-slate-300 transition-all p-3 cursor-pointer group">
      ...
      <div className="flex items-center gap-1.5 mb-2.5">
        <div className="w-4 h-4 rounded bg-slate-100 flex items-center justify-center text-slate-600 font-bold text-[8px] shrink-0">
      ...
      <div className="flex items-center gap-1 flex-wrap mb-2">
        ...
        {card.health && (
          <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${HEALTH_COLORS[card.health]}`}>{card.health}</span>
        )}
      </div>
        ...
        <div className="flex gap-3 h-full px-3 py-3" style={{ minWidth: "max-content" }}>
          {STAGES.map(stage => {
            ...
            return (
              <div
                key={stage.key}
                className={`flex flex-col rounded-xl border ${stage.borderColor} shadow-sm transition-all ${isCollapsed ? "w-10" : "w-56"}`}
                style={{ minHeight: 0 }}
              >
                {/* ── Column header — STICKY, never scrolls away ───────── */}
                <div className={`${stage.color} ${stage.textColor} rounded-t-xl px-3 py-2.5 shrink-0 flex items-center justify-between`}>
                  {isCollapsed ? (
                    <button
                      onClick={() => setCollapsed(c => ({ ...c, [stage.key]: false }))}
                      className="w-full flex flex-col items-center gap-1 opacity-90 hover:opacity-100"
                      title={stage.label}
                    >
                      <span className={`w-2 h-2 rounded-full bg-white/70`} />
                      <span className="text-[9px] font-semibold tracking-wide writing-mode-vertical opacity-80">
                        {stage.label}
                      </span>
                    </button>
                  ) : (
                    <>
                      <div className="flex items-center gap-1.5 min-w-0">
                        <span className={`w-2 h-2 rounded-full ${stage.dot} ring-1 ring-white/30`} />
                ...
                {/* Column sub-header: value sum */}
                {!isCollapsed && (
                  <div className={`${stage.bgCol} px-3 py-1.5 border-b ${stage.borderColor} shrink-0`}>
                    <span className="text-[10px] font-semibold text-slate-500">{stage.value} pipeline</span>
                  </div>
                )}

                {/* ── Cards area — this column scrolls independently ────── */}
                {!isCollapsed && (
                  <div className={`flex-1 overflow-y-auto ${stage.bgCol} rounded-b-xl`} style={{ minHeight: 0 }}>

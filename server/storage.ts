...
export class DatabaseStorage implements IStorage {
  ...
  async seedTestEmails(): Promise<void> {
    ...
    for (const seed of seeds) {
      try {
        await db.insert(emailMessages).values(seed as any).onConflictDoNothing();
      } catch (e) {
        // skip on conflict
      }
    }
